...
/**
 * GET /api/product-admin/rbac/admins
 * List all product admins (superadmin only)
 */
export async function getAllAdmins(req: AuthRequest, res: Response): Promise<void> {
  try {
    if (!req.user?.userId) {
...
export async function promoteAdmin(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    const { adminId, newRole } = validation.data;
    const targetRole = newRole || "superadmin";
    ...
    if (!["superadmin", "admin"].includes(adminToPromote.role)) {
      res.status(400).json({ error: "User is not a product admin" });
      return;
    }
    ...
    const updated = await prisma.user.update({
      ...
      data: {
        role: targetRole,
        // If promoting to superadmin, they don't need collegeId
        ...(targetRole === "superadmin" && { collegeId: null }),
      },
      select: {
        id: true,
        email: true,
...
// ─── Demote Admin (to admin) ──────────────────────────────────────────────────

/**
 * POST /api/product-admin/rbac/demote
 * Demote a superadmin to admin and assign to a college (superadmin only)
 */
export async function demoteAdmin(req: AuthRequest, res: Response): Promise<void> {
  try {
    if (!req.user?.userId) {
    ...
    if (!collegeId) {
      res.status(400).json({ error: "College ID is required for demoting to admin" });
      return;
    }
    ...
    if (adminToDemote.role !== "superadmin") {
      res.status(400).json({ error: "Only superadmins can be demoted" });
      return;
    }
    ...
    res.status(200).json({
      success: true,
      message: "Admin demoted to admin role successfully",
      admin: updated,
    });
  } catch (err: any) {
    console.error("[rbac/demote] Error:", err);
